...
var c1 = 0, c2 = 0, c4 = 0, c5 = 0, c6 = 0, c7 = 0;
function creditone()
	{
        	var one=0;
...
function depositequally()
	{
	var sum = credittext1.value;
	var c = credittext2.value;
	var p = depositselect.value;
	var d = credittext4.value
	if (sum == "0" || c == "0" || d == "0")
		{
		...
		if (p == "0")
			{
			var interestsum = (parseFloat(sum) * parseFloat(c) * (parseFloat(d) / 12)) / 100;
			var four = parseFloat(sum) + parseFloat(interestsum);
			credittext6.value = interestsum;
			credittext5.value = credittext2.value;
			}
		else
			{
			var one = 1 + (parseFloat(c) / (100 * parseFloat(p)));
			var onehalf = parseFloat(p) * (parseFloat(d) / 12);
			var two = Math.pow(one, onehalf);
			var three = (parseFloat(two) - 1) * (100 / (parseFloat(d) / 12));
			var interestsum = ((sum * three / 100) * (parseFloat(d) / 12)).toFixed(2);
			credittext6.value = interestsum;
			credittext5.value = three.toFixed(2);
			}
		var four = parseFloat(sum) + parseFloat(interestsum);
		credittext7.value = four;
		}
	}
